...
rBLAST_single_result <- function(results_table, bam_file, which_result = 1, num_reads = 100, hit_list = "1", db_path) {
  final_df <- NULL
  res <- tryCatch( #If any errors, should just skip the organism
    {
      id <- results_table[which_result,1]
      ...
      ti <- strsplit(id, "|", fixed = TRUE)[[1]][2]
      if (!quiet) message("Current ti: ", ti)
      fasta_seqs <- Biostrings::RNAStringSet(getSeqs(id = id, bamFile = bam_file, n = num_reads))
